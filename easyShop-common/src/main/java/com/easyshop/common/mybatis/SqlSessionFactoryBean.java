 ...
 */
package com.easyshop.common.mybatis;
...
/**
 * @description 自定义SqlSessionFactoryBean,指定扫描classpath下的mapper文件
...
public class SqlSessionFactoryBean implements
	...
	private void autoParse(Configuration configuration) throws IOException {
		...
		List<String> lists = new ArrayList<String>();
		String scanPath = scanMappingResourceDir + "/*" + ((null != this.suffix && this.suffix.trim().length() > 0) ? this.suffix.trim() : "") + ".xml";
